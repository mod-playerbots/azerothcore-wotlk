...
// Playerbots: public wrappers, the originals are inline in this file.
bool Guild::HasRankRight(Player* player, uint32 right) const { return _HasRankRight(player, right); }
bool Guild::MemberHasTabRights(ObjectGuid guid, uint8 tabId, uint32 rights) const { return _MemberHasTabRights(guid, tabId, rights); }
uint32 Guild::GetRankRights(uint8 rankId) const { return _GetRankRights(rankId); }
