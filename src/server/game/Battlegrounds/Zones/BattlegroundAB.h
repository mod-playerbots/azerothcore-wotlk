...
public:
    ...
    // Playerbots: node capture state, the nested CapturePointInfo stays private.
    [[nodiscard]] uint8 GetCapturePointState(uint32 node) const { return _capturePointInfo[node]._state; }
private:
