                            ...
                            }).Schedule(25s, [this](TaskContext context)
                                {
                                ...
                                }).Schedule(1min, [this](TaskContext context)
                                    {
                                    ...
                                    }).Schedule(33s, [this](TaskContext context)
                                        {
                                            ...
                                            context.Repeat(15s);
                                        }).Schedule(8min, [this](TaskContext /*context*/)
                                            {
                                                if (!_inBerserk)
                                                {
...
class spell_old_mark_of_kazzak_aura : public AuraScript
{
    PrepareAuraScript(spell_old_mark_of_kazzak_aura);
    ...
    {
        // DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_old_mark_of_kazzak_aura::CalculateAmount, EFFECT_0, SPELL_AURA_PERIODIC_MANA_LEECH);
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_old_mark_of_kazzak_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_MANA_LEECH);
    }
};
...
{
    new boss_kazzak();
    RegisterSpellScript(spell_old_mark_of_kazzak_aura);
}
