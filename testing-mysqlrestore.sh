...
#
# This script finds the latest backups and restores the auth, characters, and world databases
# to the testing environment, keeping player accounts, characters, and realm data in sync.
#
...
# Databases to sync (auth for accounts, characters for character data, world for realm data)
...
get_testing_container() {
	...
	elif docker ps -q -f name=ac-database 2>/dev/null | grep -q .; then
...
log ""
log "========================================="
log "Rebuilding containers to apply module database modifications"
log "========================================="

# Stop all containers
log "Stopping all containers..."
docker stop $(docker ps -a -q) 2>&1 | tee -a "$LOG_FILE"

# Remove all containers (including volumes to ensure clean state)
log "Removing all containers and volumes..."
docker rm --volumes $(docker ps -a -q) 2>&1 | tee -a "$LOG_FILE"
...
if [ $? -eq 0 ]; then
	log "SUCCESS: Containers rebuilt and started"
	log "Testing modules will now apply their database modifications"
else
	log "ERROR: Failed to rebuild containers"
	exit 1
fi

log ""
log "========================================="
log "Full restore and rebuild completed successfully"
log "========================================="
